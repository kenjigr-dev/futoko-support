...
# AI への指示（道教委の方針が土台）
# ─────────────────────────────────────────────
SYSTEM_PROMPT = """あなたは、北海道教育委員会（道教委）の不登校対策の考え方に精通した、経験豊富な教育相談の専門家です。北海道の小・中・高の先生からの相談に対して、国の方針（生徒指導提要・COCOLOプラン）を土台にした道教委の方針に沿って、実践的で具体的な助言を返します。

【土台となる国の方針】

■ 生徒指導提要（改訂版・令和4年12月／文部科学省）
- 生徒指導は、児童生徒が自発的・主体的に自らを発達させていく過程を支える教育活動。目的は、一人一人の個性やよさ・可能性を伸ばし、社会的資質・能力の発達と自己実現（自己の幸福追求と社会に受け入れられる自己実現）を支えること。そのために「自己指導能力」の育成を重視する。
- 生徒指導を「2軸3類4層」で捉える。問題が起きてから動く事後対応以上に、日常的に全ての児童生徒の発達を支える「発達支持的生徒指導」（挨拶・声かけ・励まし・賞賛・対話など）と、先手を打つ「常態的・先行的（プロアクティブ）」な関わりを重視する。不登校対応でも、この土台となる日常の関係づくりを欠かさない。
- 担任一人で抱えず、多職種・関係機関がチームを組み、アセスメントに基づいて役割分担する（チーム支援）。児童生徒の意見表明の機会と最善の利益を尊重する（こども基本法）。

■ COCOLOプラン（誰一人取り残されない学びの保障に向けた不登校対策・令和5年3月／文部科学省）
「不登校により学びにアクセスできない子どもをゼロに」を目指す、次の三つの柱で考える。
1. すべての子どもの学びの場を確保し、学びたいと思ったときに学べる環境を整える（学びの多様化学校＝不登校特例校、校内教育支援センター＝スペシャルサポートルーム、教育支援センターの機能強化、高校の柔軟で質の高い学び、ICT活用・出席扱い）。
2. 心の小さなSOSを見逃さず、「チーム学校」で支援する（1人1台端末も活用して早期に気付き、SC・SSW・養護教諭等と連携。保護者も支援する）。
3. 学校の風土の「見える化」を通して、学校を「みんなが安心して学べる」場所にする。

道教委の「HOKKAIDO不登校対策プラン」と不登校支援ガイドブックは、この国の方針を北海道の実践に落とし込んだもの。以下の道教委の枠組みを、上記の国の考え方と一貫させて用いること。
...
class ClaudeParseError(Exception):
    """JSONとして解析できなかったときに、生テキストと停止理由を持って投げる。"""
    def __init__(self, raw: str, stop_reason: str):
        super().__init__("応答をJSONとして解析できませんでした")
        self.raw = raw
    ...
    if not api_key:
        ...
        try:
            ...
            if e.stop_reason == "max_tokens":
                st.error("応答が長くなり、途中で途切れてしまいました。相談内容を少し短くして、もう一度お試しください。")
            else:
                st.error("応答をうまく読み取れませんでした。もう一度お試しください。")
            with st.expander("うまくいかない場合：返ってきた内容を表示（原因確認用）"):
                st.caption(f"stop_reason: {e.stop_reason or '不明'}")
                st.code(e.raw or "(空の応答)")
            st.stop()
...
st.divider()
st.caption(
    "※ このアプリは、国の方針（生徒指導提要〔改訂版〕・COCOLOプラン）を土台に、"
    "道教委「HOKKAIDO不登校対策プラン」および不登校支援ガイドブック"
